...
function getMaxMulti(rarity) {
    ...
    if(rarity === 'Special') return 50; // Allows up to 50x clusters for Debris/FPS in the search bar
    return 1;
}
...
function generateOreTable() {
    ...
    ores.forEach(ore => {
        // Handle the non-ore special signatures (Debris, FPS Mineables, etc.)
        if (!ore.rarity) {
            htmlSpecials += `
    ...
    // Output regular ores first, then append the special items at the bottom
